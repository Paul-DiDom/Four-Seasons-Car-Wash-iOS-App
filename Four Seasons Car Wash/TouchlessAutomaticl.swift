//
//  TouchlessAutomaticl.swift
//  Four Seasons Car Wash
//
//  Created by Paul Di Domenico on 2017-01-05.
//  Copyright © 2017 Tech1st Wash Systems. All rights reserved.
//

import UIKit

class TouchlessAutomaticl: UIViewController ,  UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @objc func checkForMsg(){
        if (self.msgToShow){
            let alert = UIAlertController(title: "Message", message: self.m, preferredStyle: .alert)
    ...
    func getMessage() {
        if (hasRunOnce == false) {
            hasRunOnce = true
            let myUrl = NSURL(string: service + "cpmsg")!
            ...
            let task = URLSession.shared.dataTask(with: request as URLRequest) {
                ...
                do {
                    let myJSON = try JSONSerialization.jsonObject(with: data!, options: .mutableLeaves) as? NSDictionary
                    if let parseJSON = myJSON {
                        let myResult = parseJSON["cpmsgResult"] as? String
                        //print(myResult as Any)
                        if ((myResult?.starts(with: "ON,")) == true){
                            self.m = myResult!.replacingOccurrences(of: "ON,", with: "")
                            self.msgToShow = true
                        }
                    }
                }
            ...
            DispatchQueue.main.async(execute: { () -> Void in
                Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(self.checkForMsg), userInfo: nil, repeats: false)
            })
        }
    ...
    func getMenu() {
        pleaseWait()
        let myUrl = NSURL(string: service + "lmenu")!
        let request = NSMutableURLRequest(url:myUrl as URL);
        request.httpMethod = "POST";
        request.setValue("application/json; charset=utf-8", forHTTPHeaderField: "Content-Type")
        let dictionary = [String: String]()
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest) {
            ...
            do {
                let myJSON = try JSONSerialization.jsonObject(with: data!, options: .mutableLeaves) as? NSDictionary
                if let parseJSON = myJSON {
                    let myResult = parseJSON["lmenuResult"] as? String
                    ///      print(myResult!)
