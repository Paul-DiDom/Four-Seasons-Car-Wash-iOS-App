...
class coinBay2: UIViewController , UIPickerViewDataSource, UIPickerViewDelegate

{
    ...
    override func viewDidLoad() {
        ...
        devicePickerOptions = ["Choose your "+device,"Use "+deviceUC+" 1", "Use "+deviceUC+" 2", "Use "+deviceUC+" 3", "Use "+deviceUC+" 4", "Use "+deviceUC+" 5", "Use "+deviceUC+" 6", "Use "+deviceUC+" 7", "Use "+deviceUC+" 8", "Use "+deviceUC+" 9", "Use "+deviceUC+" 10", "Use "+deviceUC+" 11", "Use "+deviceUC+" 12"];
        
        amountPickerOptions = ["Select an amount","$4.00", "$5.00","$6.00", "$7.00", "$8.00", "$9.00","$10.00","$11.00","$12.00", "$13.00", "$14.00", "$15.00", "$16.00", "$17.00", "$18.00", "$19.00", "$20.00"];
        devicePicker.tag = 1
        amountPicker.tag = 2
    ...
    override func viewDidAppear(_ animated: Bool) {
        let didPayPet = UserDefaults.standard.object(forKey: "coinAdd") as! Bool
        if (didPayPet) {
            amountPickerOptions = ["Select an amount", "$1.00", "$2.00", "$3.00", "$4.00", "$5.00", "$6.00", "$7.00", "$8.00", "$9.00", "$10.00",
                                   "$11.00","$12.00", "$13.00", "$14.00", "$15.00", "$16.00", "$17.00", "$18.00", "$19.00", "$20.00"];
            amountPicker.reloadAllComponents();
            amountPicker.selectRow(1, inComponent: 0, animated: true)
            amountChosen = "1.00"
            let alert = UIAlertController(title: "Add Time", message: "You can add more time while the clock is counting down. If the time clock runs out you must add $4 again to start.", preferredStyle: .alert)
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int)
    {
        ...
        if (deviceChosen.contains("0")) || amountChosen.contains("amount") {
            btnStart.isHidden = true
            if deviceChosen.contains("10"){
                if (amountChosen.contains("amount") == false){
